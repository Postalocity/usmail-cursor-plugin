...
The human approves on the app, or explicitly grants postage at the cap the human chooses (uncapped only if the human explicitly asks for no cap) and then sends Looks good after the proof. The agent does not fund the meter, run card deposits, or do EDDM.
...
- Ask about a postage spend grant early. The grant stays off by default; the agent turns it on only when the human explicitly asks.
...
5. A spend grant is optional and off until the human asks for one. The agent turns a grant on only when the human explicitly asks, at the cap the human chooses (uncapped only if the human explicitly asks for no cap). It never deposits funds, and submits only after the human reviews the proof and replies Looks good.
